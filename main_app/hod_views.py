...
def add_staff_from_xlsx(request):
    if request.method == "POST":
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):  # skip header
            # Ensure there are exactly 7 values
            if len(row) < 7:
                messages.error(request, f"Incomplete data in row: {row}")
                continue

            first_name, last_name, address, email, gender, password, course_name = row
            ...
            try:
                ...
                messages.error(request, f"Error creating user {email}: {str(e)}")

        messages.success(request, "Staff accounts created successfully from XLSX.")
        return redirect('add_staff')

    return render(request, 'hod_template/add_staff_from_xlsx.html')
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ...
                user = CustomUser.objects.create_user(
                    email=email, password=password, user_type=4,  # user_type 4 is for Parent
                ...
                Parent.objects.create(admin=user)

                messages.success(request, "Parent Successfully Added")
                return redirect(reverse('add_parent'))  # Redirect to the same page to add another parent

            except Exception as e:
                messages.error(request, "Could Not Add Parent: " + str(e))
        else:
            messages.error(request, "Please fulfil all requirements")
...
def add_students_from_xlsx(request):
    if request.method == 'POST' and request.FILES['xlsx_file']:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):  # skip header row
            ...
            try:
                user = CustomUser.objects.create_user(
                    email=email, password=password, user_type=3,  # assuming user_type 3 is for students
                    first_name=first_name, last_name=last_name
                )
                ...
                messages.error(request, f"Error adding student {first_name} {last_name}: {str(e)}")

        return redirect('add_student')  # Redirect to the page after processing the file
...
def parent_feedback_message(request):
    # Display feedbacks for HOD
    if request.method != 'POST':
        feedbacks = ParentFeedback.objects.all()  # Get all parent feedbacks
        context = {
            'feedbacks': feedbacks,
    ...
    # Handle the reply action from HOD
    else:
        feedback_id = request.POST.get('id')  # Get feedback ID from POST
        try:
            feedback = get_object_or_404(ParentFeedback, id=feedback_id)
            reply = request.POST.get('reply')  # Get the reply from the form
            feedback.reply = reply  # Update the feedback with the reply
            feedback.save()  # Save the feedback with the reply
            return HttpResponse("True")  # Return success response
        except Exception as e:
            return HttpResponse("False")  # Return failure response
...
def admin_notify_parent(request):
    parents = CustomUser.objects.filter(user_type=4)  # Fetch all Parent users
    context = {
...
def send_parent_notification(request):
    if request.method == "POST":
        ...
        try:
            parent = Parent.objects.get(admin__id=parent_id)
